...
// Tarihleri epoch zamanına çeviren fonksiyon
...
int main() {
    ...
    printf("Uye No: "); 
	scanf("%d", &kullanici.uyeNo);
    
    do {
        printf("\nOdunc aldiginiz tarihi giriniz:\n");
        printf("Yil: "); 
		scanf("%d", &oduncTarihi.yil);
        if (oduncTarihi.yil < 1970) 
		printf("Hata! Yil 1970'ten kucuk olamaz! Tekrar giriniz.\n");
    } while (oduncTarihi.yil < 1970);
    
    do {
        printf("Ay (1-12): "); 
		scanf("%d", &oduncTarihi.ay);
        if (oduncTarihi.ay < 1 || oduncTarihi.ay > 12) 
		printf("Hata! Ay 1 ile 12 arasinda olmalidir! Tekrar giriniz.\n");
    } while (oduncTarihi.ay < 1 || oduncTarihi.ay > 12);
    
    do {
        printf("Gun: "); 
		scanf("%d", &oduncTarihi.gun);
        if (oduncTarihi.gun < 1 || oduncTarihi.gun > 31) 
		printf("Hata! Gun 1 ile 31 arasinda olmalidir! Tekrar giriniz.\n");
    } while (oduncTarihi.gun < 1 || oduncTarihi.gun > 31);
    
    do {
        printf("\nIade tarihini giriniz:\n");
        printf("Yil: "); 
		scanf("%d", &iadeTarihi.yil);
        if (iadeTarihi.yil < 1970) 
		printf("Hata! Yil 1970'ten kucuk olamaz! Tekrar giriniz.\n");
    } while (iadeTarihi.yil < 1970);
    
    do {
        printf("Ay (1-12): "); 
		scanf("%d", &iadeTarihi.ay);
        if (iadeTarihi.ay < 1 || iadeTarihi.ay > 12)
		printf("Hata! Ay 1 ile 12 arasinda olmalidir! Tekrar giriniz.\n");
    } while (iadeTarihi.ay < 1 || iadeTarihi.ay > 12);
    
    do {
        printf("Gun: ");
		scanf("%d", &iadeTarihi.gun);
        if (iadeTarihi.gun < 1 || iadeTarihi.gun > 31) 
		printf("Hata! Gun 1 ile 31 arasinda olmalidir! Tekrar giriniz.\n");
